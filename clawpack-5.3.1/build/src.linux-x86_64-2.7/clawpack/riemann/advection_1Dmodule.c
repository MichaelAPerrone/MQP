...
static PyObject *advection_1D_error;
static PyObject *advection_1D_module;
...
#define CHECKSCALAR(check,tcheck,name,show,var)\
  if (!(check)) {\
    ...
    PyErr_SetString(advection_1D_error,errstring);\
    /*goto capi_fail;*/\
  } else 
#ifdef DEBUGCFUNCS
...
static int int_from_pyobj(int* v,PyObject *obj,const char *errmess) {
  ...
  }
  {
    ...
    if (err==NULL) err = advection_1D_error;
...
/************************************ rp1 ************************************/
static char doc_f2py_rout_advection_1D_rp1[] = "\
...
/* extern void F_FUNC(rp1,RP1)(int*,int*,int*,int*,int*,int*,double*,double*,double*,double*,double*,double*,double*,double*); */
static PyObject *f2py_rout_advection_1D_rp1(const PyObject *capi_self,
                           PyObject *capi_args,
...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOOOOOOOOO|OO:advection_1D.rp1",\
...
/*frompyobj*/
  /* Processing variable maxmx */
    f2py_success = int_from_pyobj(&maxmx,maxmx_capi,"advection_1D.rp1() 1st argument (maxmx) can't be converted to int");
  if (f2py_success) {
  /* Processing variable auxl */
    f2py_success = double_from_pyobj(&auxl,auxl_capi,"advection_1D.rp1() 7th argument (auxl) can't be converted to double");
  if (f2py_success) {
  /* Processing variable maux */
    f2py_success = int_from_pyobj(&maux,maux_capi,"advection_1D.rp1() 2nd argument (maux) can't be converted to int");
  if (f2py_success) {
  /* Processing variable mbc */
    f2py_success = int_from_pyobj(&mbc,mbc_capi,"advection_1D.rp1() 3rd argument (mbc) can't be converted to int");
  if (f2py_success) {
  /* Processing variable auxr */
    f2py_success = double_from_pyobj(&auxr,auxr_capi,"advection_1D.rp1() 8th argument (auxr) can't be converted to double");
  if (f2py_success) {
  /* Processing variable mx */
    f2py_success = int_from_pyobj(&mx,mx_capi,"advection_1D.rp1() 4th argument (mx) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_ql_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_1D_error,"failed in converting 5th argument `ql' of advection_1D.rp1 to C/Fortran array" );
  } else {
    ql = (double *)(capi_ql_tmp->data);

  /* Processing variable meqn */
  if (meqn_capi == Py_None) meqn = shape(ql,0); else
    f2py_success = int_from_pyobj(&meqn,meqn_capi,"advection_1D.rp1() 1st keyword (meqn) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_wave_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_1D_error,"failed in converting 9th argument `wave' of advection_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_amdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_1D_error,"failed in converting 11st argument `amdq' of advection_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_apdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_1D_error,"failed in converting 12nd argument `apdq' of advection_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_qr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_1D_error,"failed in converting 6th argument `qr' of advection_1D.rp1 to C/Fortran array" );
  } else {
    qr = (double *)(capi_qr_tmp->data);

  /* Processing variable mwaves */
  if (mwaves_capi == Py_None) mwaves = shape(wave,1); else
    f2py_success = int_from_pyobj(&mwaves,mwaves_capi,"advection_1D.rp1() 2nd keyword (mwaves) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_s_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_1D_error,"failed in converting 10th argument `s' of advection_1D.rp1 to C/Fortran array" );
  } else {
...
static FortranDataDef f2py_routine_defs[] = {
  {"rp1",-1,{{-1}},0,(char *)F_FUNC(rp1,RP1),(f2py_init_func)f2py_rout_advection_1D_rp1,doc_f2py_rout_advection_1D_rp1},
...
#if PY_VERSION_HEX >= 0x03000000
#define RETVAL m
PyMODINIT_FUNC PyInit_advection_1D(void) {
#else
#define RETVAL
PyMODINIT_FUNC initadvection_1D(void) {
#endif
  int i;
  PyObject *m,*d, *s;
#if PY_VERSION_HEX >= 0x03000000
  m = advection_1D_module = PyModule_Create(&moduledef);
#else
  m = advection_1D_module = Py_InitModule("advection_1D", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module advection_1D (failed to import numpy)"); return RETVAL;}
...
#endif
    "This module 'advection_1D' is auto-generated with f2py (version:2).\nFunctions:\n"
"  rp1(maxmx,maux,mbc,mx,ql,qr,auxl,auxr,wave,s,amdq,apdq,meqn=shape(ql,0),mwaves=shape(wave,1))\n"
"COMMON blocks:\n""  /cparam/ u\n"".");
  PyDict_SetItemString(d, "__doc__", s);
  advection_1D_error = PyErr_NewException ("advection_1D.error", NULL, NULL);
  Py_DECREF(s);
...
#ifdef F2PY_REPORT_ATEXIT
  if (! PyErr_Occurred())
    on_exit(f2py_report_on_exit,(void*)"advection_1D");
#endif
